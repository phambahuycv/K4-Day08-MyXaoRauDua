...
def retrieve(
    ...
    if best_score < score_threshold or not final_results:
        print(f"  ⚠ Semantic best score ({best_score:.3f}) < threshold ({score_threshold}). Fallback sang PageIndex!")
...
if __name__ == "__main__":
    test_queries = [
        "Chính sách đổi trả sản phẩm FPT Shop",
        "Cách tạo cửa hàng trên TikTok Shop",
        "xyzabc123nonsense_query_test",  # Query rác -> trigger fallback
    ]
    ...
    for q in test_queries:
        ...
        for i, r in enumerate(results, 1):
            print(f"  {i}. [{r['score']:.4f}] [{r.get('source', 'unknown')}] {r['content'][:80]}...")
