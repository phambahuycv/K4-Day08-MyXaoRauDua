...
def retrieve(
    ...
    if not isinstance(query, str) or not query.strip() or top_k <= 0:
        return []
    # Chroma's embedded local server and the in-memory BM25 index are initialized
    # independently; after warm-up these calls can be parallelized by the caller.
    ...
    # Fallback decisions intentionally use the original cosine score, not RRF.
