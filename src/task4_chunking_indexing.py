...
def load_documents() -> list[dict]:
    ...
    if not STANDARDIZED_DIR.exists():
        print(f"[WARN] Thư mục {STANDARDIZED_DIR} chưa tồn tại!")
        return documents
...
def chunk_documents(documents: list[dict]) -> list[dict]:
    ...
    try:
        ...
        # Fallback pure python chunker if langchain_text_splitters is not available
        chunks = []
        for doc in documents:
            text = doc["content"]
            start = 0
...
def embed_chunks(chunks: list[dict]) -> list[dict]:
    """
    Embed toàn bộ chunks bằng model đã chọn (sentence-transformers).
    ...
    try:
        ...
        print(f"[WARN] Không tải được model {EMBEDDING_MODEL}: {e}. Fallback về all-MiniLM-L6-v2")
